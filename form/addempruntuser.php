...
$empruntuser=$pdo->fetch("SELECT * FROM `empruntuser` ");
...
if(isset($_GET['id']) && $_GET['id']=='modify'){
  ...
  $empruntuser=$pdo->fetch("SELECT * FROM `empruntuser` WHERE id_empruntuser=$matricul");
...
?>
<!-- le formulaire d'emprunt et des modification des emprunt -->
  <div class="form container">
    <div class="">
      <div class="">
        
        <h1><?= isset($modify)?"Modifier":"Ajouter"?> un livre</h1> <!--vrification d'existance de modif-->
        <form action="../crud/addcrud.php" method="POST" class="m-3">
          ...
          <div class="form-group">
           ...
           <option value="<?=isset($ajour)?$doc['id_livre']:$livres['id_livre']?>"
                <?= (isset($modify) && $livres['id_livre']==$empruntuser['id_livres'])?"selected":'' ?>
          ...
          <div class="form-group">
            ...
            <input type="date" name="dates" class="form-control" value="<?=isset($modify)?$empruntuser['date_sortie']:""?>" required><!--prée remplissement des dates si le modify exite-->
          </div>
          <div class="form-group">

            <label for="njd">Date de rendu</label>
            <input type="date" name="dater" class="form-control" value="<?= isset($modify)?$empruntuser['date_rendu']:""?>"><!--prée remplissement des dates si le modify exite-->
            
          </div>
          <br>
          <button type="submit" class="btn btn-primary"><?= isset($modify)?"Modifier":"Ajouter"?></button>
          <input type="hidden" name="action" value="<?= isset($modify)?"editempruntuser":"addempruntuser"?>"><!--envoir de l'action a effectuer-->
          <input type="hidden" name="matricul" value="<?= isset($modify)? $matricul:$matriculuser?>"><!--envoir de l'identifiant du l'utilisateur-->
        </form>
      </div>
    </div>
  </div>
